...
// 4	陈宫	智迟	翻牌技，令一名武将获得一个技能：每次主动攻击后恢复3点生命值
type SkillZhiChi struct {
	HeroSkill
	targetCards []*Card
}

func (ss *SkillZhiChi) CanUse() ([]*Card, error) {
...
func (s *SkillZhiChi) OnFaceUp(card *Card) {
	if s.targetCards == nil {
		return
	}
	for _, v := range s.targetCards {
		StartGetBuff(v, s.GetBuffId0(), gameconf.ExpireTyp_ETInvalid, 0, card)
	}
