...
type msg interface {
	marshal() (ext interface{}, msgid uint32, data []byte, err error)
}
...
func (m *bytesMsg) marshal() (interface{}, uint32, []byte, error) {
...
func (m *protoMsg) marshal() (interface{}, uint32, []byte, error) {
...
func (c *connection) WriteMsg(ext interface{}, msg interface{}) error {
...
func (c *connection) WriteBytes(ext interface{}, msgid uint32, data []byte) error {
...
func (c *connection) readLoop() {
	...
	for {
		...
		msgid, extData, msgData, err := c.msgPackager.ReadMsg(r, c.crypto)
		if c.isClosed() {
			break
		}
		if err != nil {
	...
	if c.isServer {
		logger.WithField("name", c.Name()).Debug("Conn exit read loop")
	} else {
		logger.WithField("name", c.Name()).Debug("Conn exit read loop")
	}
}
