...
// BytesHandler ...
type BytesHandler func(conn connection.Connection, ext interface{}, msgid uint32, bytes []byte)

// MsgHandler ...
type MsgHandler func(conn connection.Connection, ext interface{}, msgid uint32, bytes []byte,msg interface{})

// MsgProcessor 消息处理者
type MsgProcessor interface {
	// on read gorutine decode扩展数据
	OnDecodeExt(extData []byte) (ext interface{}, err error)
	// on write gorutine encode扩展数据
	OnEncodeExt(ext interface{}) (extData []byte, err error)
	// on read gorutine
	// 连接
	...
	// 消息处理函数
	OnMessage(conn connection.Connection, ext interface{}, msgid uint32, msgData []byte) error
}
...
// RegisterMessage ...
func RegisterMessage(msg interface{}) (uint32, error) {
...
func OnUnmarshal(id uint32, data []byte) (interface{}, error) {
...
func OnMarshal(msg interface{}) (uint32, []byte, error) {
	//
	msgType := reflect.TypeOf(msg)
