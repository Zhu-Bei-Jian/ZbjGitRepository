...
func newTCPConn(name string, conn net.Conn, isServer bool, tcpConnWriteChanLen int, bCloseBuffFull bool, msgPackager msgpackager.MsgPackager, msgProcessor msgprocessor.MsgProcessor, crypto crypto.Crypto) *Conn {
	tcpConn := new(Conn)
...
func (tcpConn *Conn) doWrite(ext interface{}, msgid uint32, msg interface{}) error {
...
// WriteMsg ...
func (tcpConn *Conn) WriteMsg(ext interface{}, msg interface{}) error {
	if msg == nil {
		return ErrMsgIsNil
	}
	if tcpConn.isClosed() {
...
// WriteBytes ...
func (tcpConn *Conn) WriteBytes(ext interface{}, msgid uint32, bytes []byte) error {
	if tcpConn.isClosed() {
		return ErrClosed
	}
...
func (tcpConn *Conn) ReadLoop() {
	...
	for {
		msgid, extData, msgData, err1 := tcpConn.msgPackager.ReadMsg(tcpConn.conn, tcpConn.crypto)
		if err1 != nil {
			if err1 != io.EOF {
				if tcpConn.isServer {
					logger.WithFields(logrus.Fields{
						"name": tcpConn.Name(),
					}).WithError(err1).Debug("Conn read message error")
				} else {
					logger.WithFields(logrus.Fields{
						"name": tcpConn.Name(),
